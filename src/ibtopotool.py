...
ClusterShell, "pip install ClusterShell" or "apt install python3-clustershell" on Debian/Ubuntu""")
        raise
    ...
    for n, nbrs in g.adjacency():
        if g.nodes[n]['type'] == 'Switch':
            ...
            if len(switches) > 0:
                switches.sort()
                switchstring = " Switches=" + str(NodeSet.fromlist(switches))
            nodestr = ''
            if len(nodes) > 0:
                nodes.sort()
                nodestr = " Nodes=" + str(NodeSet.fromlist(nodes))
