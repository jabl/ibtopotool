...
# Generate a graphviz dot file, or slurm topology.conf from IB topology
...
def gen_slurm(graph, desc, out):
    """
    graph: Connectivity graph
    desc: {lid:Description} pairs
    ...
    for k, v in desc.items():
        if v['type'] == 'Switch':
            ...
            for k2, v2 in graph[k].items():
                if desc[k2]['type'] == 'Switch':
                    switches.append("s%d" % k2)
                else:
                    nodename = desc[k2]['desc'].split()[0]
